...
feature -- evoluzione della statechart
	...
	genitore_piu_grande (transizione: TRANSIZIONE): STATO
			-- Arianna Calzuola & Riccardo Malandruccolo 22/05/2020
			-- restituisce l'antenato pi� grande che contiene stato_corrente ed � contenuto nel contesto
		local
	...
	--qual'ora vi siano stati gi� attivi non da luogo a configurazioni non corrette
		local
			i: INTEGER
		do
			target.set_attivo
			if attached {STATO_AND} target.genitore as sgt and then not sgt.attivo then --se lo stato genitore � un AND scorro i suoi figli
				from
					i := sgt.initial.lower
				until
					i = sgt.initial.upper + 1
				loop
					if not sgt.initial [i].is_equal (target) then -- ho gi� aggiunto il target alla conf corrente
							--MODIFICHE FORK
	...
	aggiungi_sottostati (stato: STATO; prossima_conf_base: ARRAY [STATO])
		...
		do
			if stato.ha_sottostati_attivi then --prima discriminante: ha sottostati attivi? Se SI
				stato.set_attivo
				esegui_onentry (stato)
				if attached {STATO_AND} stato as s_and then --se lo stato � un AND
					across
						s_and.figli as fsa
					loop
