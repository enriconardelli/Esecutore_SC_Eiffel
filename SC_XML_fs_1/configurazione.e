		...
		do
			...
			if valore_booleano (espressione) then
				...
				debug ("SC_inizializza_variabili")
					print ("Booleano: " + variabile + " = " + variabili.booleane [variabile].out + "%N")
				end
			elseif valore_intero (espressione) then
				variabili.intere.extend (espressione.to_integer, variabile)
				debug ("SC_inizializza_variabili")
					print ("Intero: " + variabile + " = " + variabili.intere [variabile].out + "%N")
				end
			else
		...
		do
			...
			if ancore_multiple_compatibili (tutte_le_sorgenti) then
				...
				debug ("sc_transizioni_legali")
					print (" - Transizione legale da >|" + sorgente.id + "|< a >|" + destinazione.id + "|< %N")
				end
			else
	...
	assegna_transizione_fork (transition_element: XML_ELEMENT; destinazioni: LINKED_LIST [STATO]; sorgente: STATO)
		...
		do
			if ancore_multiple_compatibili (destinazioni) then
				...
				debug ("sc_transizioni_legali")
					print (" - Transizione legale da >|" + sorgente.id + "|< a >|" + destinazioni.first.id + "|< %N")
				end
			else
			...
			if attached {STATO_AND} stato_mac then
				if transizione_verticale (p_sorgente, p_destinazione) then
					...
					if catena_di_paralleli (altro_stato, stato_mac) then
						Result := True
						debug ("sc_transizioni_illegali")
							print ("ERRORE: transizione illegale: transizione con MAC <parallel> in verticale e catena di <parallel> %N")
						end
					end
				else -- stato_mac è diverso da entrambi
					Result := True
					debug ("sc_transizioni_illegali")
						print ("ERRORE: transizione illegale: transizione con MAC <parallel> in orizzontale tra discendenti del MAC (attraversa la frontiera)%N")
					end
				end
			end
		end
