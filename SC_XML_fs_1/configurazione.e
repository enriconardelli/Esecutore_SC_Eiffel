	...
	istanzia_condizioni (lis_data: LIST [XML_ELEMENT])
		...
		do
			...
			condizioni.extend (true, "condizione_vuota")
				-- condizione_vuota � una condizione sempre true che si applica alle transizioni che hanno condizione void (cfr riempi_stato)
		end
...
--					end
					...
					print ("ERRORE: lo stato indicato come 'initial' non � uno degli stati in <state>")
				end
			elseif attached radice.element_by_name ("state") as st then
					-- l'assenza di "initial" � gestita scegliendo il primo dei figli se lo stato ha figli oppure scegliendo s� stesso se lo stato non ha figli
				if attached st.attribute_by_name ("id") as id then
					if attached stati.item (id.value) as df then
						imposta_stati_ricorsivo (df)
					end
				end
--			else -- TODO oppure segnalando errore se il nodo � la radice <scxml>
--				print ("ERRORE: manca lo stato 'initial' nel file e non c'� la gestione della sua assenza %N")
--					-- TODO gestire la scelta dello stato iniziale in caso di assenza dell'attributo 'initial' nel file .xml
	...
	riempi_stato (id_stato: STRING; element: XML_ELEMENT)
		local
			transition_list: LIST [XML_ELEMENT] --lista di tutto ci� che appartiene allo stato
