...
feature -- Test
	...
	t_xor_base
		local
			esecutore: ESECUTORE
		do
			argomenti [2] := argomenti [2] + "xor_eventi_1.txt"
	...
	t_impostazione_default_initial_assente
		local
			esecutore: ESECUTORE
		do
			argomenti [1] := test_data_dir + "xor_initial_assente.xml"
			argomenti [2] := argomenti [2] + "xor_initial_assente_eventi.txt"
			create esecutore.make (argomenti)
			if attached esecutore.state_chart.stati.item ("A") as st then
				if attached st.initial as df then
					assert ("ERRORE il sistema non ha impostato correttamente il default di A", df.count = 1 and df[1].id.is_equal ("A1"))
	...
	t_impostazione_default_initial_assente_2
		local
			esecutore: ESECUTORE
		do
			argomenti [1] := test_data_dir + "entrata_2_azioni_initial_assente.xml"
			argomenti [2] := argomenti [2] + "entrata_2_azioni_initial_assente_eventi.txt"
			create esecutore.make (argomenti)
			if attached esecutore.state_chart.stati.item ("A") as st then
				if attached st.initial as df then
	...
	t_xor_eventi_semplici
		local
			esecutore: ESECUTORE
		do
			argomenti [2] := argomenti [2] + "xor_eventi_2.txt"
			create esecutore.make (argomenti)
	...
	t_xor_giro_base_completo
		local
			esecutore: ESECUTORE
		do
			argomenti [2] := argomenti [2] + "xor_eventi_3.txt"
			create esecutore.make (argomenti)
