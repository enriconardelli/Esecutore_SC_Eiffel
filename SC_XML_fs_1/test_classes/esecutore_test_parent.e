...
feature -- Test
	...
	t_xor_base
		local
			esecutore: ESECUTORE
		do
			nomi_files_prova [2] := nomi_files_prova [2] + "e_xor_1.txt"
	...
	t_impostazione_default_initial_assente
		local
			esecutore: ESECUTORE
		do
			nomi_files_prova [1] := test_data_dir + "sc_xor_initial_assente.xml"
			nomi_files_prova [2] := nomi_files_prova [2] + "e_xor_1.txt"
			create esecutore.make (nomi_files_prova)
			if attached esecutore.state_chart.stati.item ("A") as st then
				if attached st.initial as df then
	...
	t_impostazione_default_initial_assente_2
		local
			esecutore: ESECUTORE
		do
			nomi_files_prova [1] := test_data_dir + "sc_entrata_2_azioni_initial_assente.xml"
	...
	t_xor_eventi_semplici
		local
			esecutore: ESECUTORE
		do
			nomi_files_prova [2] := nomi_files_prova [2] + "e_xor_2.txt"
			create esecutore.make (nomi_files_prova)
	...
	t_xor_giro_base_completo
		local
			esecutore: ESECUTORE
		do
			nomi_files_prova [2] := nomi_files_prova [2] + "e_xor_3.txt"
			create esecutore.make (nomi_files_prova)
