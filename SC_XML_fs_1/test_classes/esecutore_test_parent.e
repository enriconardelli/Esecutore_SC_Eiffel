...
feature -- Test
	...
	t_xor_base
		...
		do
			...
			assert ("ERRORE il sistema non ha terminato nello stato corretto (A1b)", esecutore.stato_corrente.count = 1 and esecutore.stato_corrente[1].id.is_equal ("A1b"))
			if attached esecutore.stato_corrente[1].stato_genitore as sp then
	...
	t_xor_eventi_semplici
		...
		do
			...
			create esecutore.make (nomi_files_prova)
			assert ("ERRORE il sistema non ha terminato nello stato corretto (B1)", esecutore.stato_corrente.count = 1 and esecutore.stato_corrente[1].id.is_equal ("B1"))
		end
	...
	t_xor_giro_base_completo
		...
		do
			...
			create esecutore.make (nomi_files_prova)
			assert ("ERRORE il sistema non ha terminato nello stato corretto (B2)", esecutore.stato_corrente.count = 1 and esecutore.stato_corrente[1].id.is_equal ("B2"))
		end
