	...
	-- Arianna Calzuola & Riccardo Malandruccolo 08/05/2020
		...
		do
  		...
  		nomi_files_prova [2] := test_data_dir + "non_determinismo_2_2_eventi.txt"
			create esecutore.make (nomi_files_prova)
			assert("ERRORE 2_2.1: il sistema entra in due stati XOR", esecutore.state_chart.conf_base.count = 1)
			assert("ERRORE 2_2.2: il sistema non entra in A", conf_has_state(esecutore.state_chart.conf_base,"A"))
		end
	...
	-- TODO: non completamente risolto
		...
		do
  		...
  		nomi_files_prova [2] := test_data_dir + "non_determinismo_2_3_eventi.txt"
			create esecutore.make (nomi_files_prova)
			assert("ERRORE 2_3.1: il sistema esce dallo stato P parallelo", esecutore.state_chart.conf_base.count = 2)
			assert("ERRORE 2_3.2: il sistema non entra in (B,P2)", conf_has_state(esecutore.state_chart.conf_base,"B") and conf_has_state(esecutore.state_chart.conf_base,"P2"))
		end
